...
为防止飞书多维表格数据丢失，系统将在后台自动将关键数据同步到 MySQL 数据库。该方案对前端完全透明，不影响用户操作体验。
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TikTok账号信息备份表';
```
...
CREATE TABLE videos (
  ...
  INDEX idx_open_id (open_id),
  INDEX idx_record_id (record_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='TikTok视频列表备份表';
```
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='素材库备份表';
```

### 4. AI素材生成 (ai_materials)
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='AI素材生成备份表';
```
...
CREATE TABLE comments (
  ...
  INDEX idx_video_id (video_id),
  INDEX idx_open_id (open_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='评论记录备份表';
```
...
- **实时同步**：在 API 端点中，每次操作飞书多维表格后，立即同步到 MySQL
- **异步备份**：使用后台任务定期全量同步，确保数据一致性
- **失败重试**：同步失败时记录到队列，后续重试

### 2. 同步时机

- **创建记录**：发布视频、创建评论、生成AI素材时
- **更新记录**：刷新Token、更新发布状态、更新任务状态时
- **删除记录**：删除评论时（软删除）

### 3. 数据恢复

提供管理接口，可以从 MySQL 恢复数据到飞书多维表格：
- 按表恢复
- 按时间范围恢复
- 按记录ID恢复
...
1. **数据安全**：防止飞书多维表格数据丢失
2. **可恢复性**：可以从 MySQL 恢复任意时间点的数据
3. **审计追踪**：操作日志记录所有数据变更
4. **性能优化**：可以从 MySQL 进行复杂查询和统计
5. **前端透明**：不影响前端操作，用户无感知
